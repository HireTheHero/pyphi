...
| N | Partitions | Network | Time (s) | Phi |
|---|---|---|---|---|
| 3 | 22 | `basic_network` (state 000) | 0.0 | 2.000000 |
| 4 | 150 | `micro` — IIT 4.0 paper (state 1111) | 0.6 | 1.331763 |
| 5 | 1,061 | `rule152` ring CA (state 00000) | 15.6 | 0.830075 |
| 6 | 7,896 | `rule152` ring CA (state 000000) | 492 | 0.276692 |

Partition count grows ~7× per node (N=5→6: ×7.4). Time per partition is roughly constant (~62 ms), so total time scales proportionally. N=7 (61,888 partitions) is projected at ~64 min sequential. At 96 CPU cores via Ray parallelism, N=6 would take ~5 s and N=7 ~40 s.
